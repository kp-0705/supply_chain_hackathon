...
class InMemoryDatabase {
  ...
  async query(text, params = []) {
    ...
    // Level 3 matching queue
    if (upper.includes('FROM DEMAND_RAW') && (upper.includes('DR.LEVEL = 2') || upper.includes('DR.LEVEL=2')) && upper.includes('PARTIALLY_ALLOCATED')) {
      const filtered = this.demand_raw.filter(d => (Number(d.level) === 2 && d.status === 'PENDING') || (Number(d.level) === 3 && d.status === 'PARTIALLY_ALLOCATED'));
    ...
    // Level 4 executive queue
    if (upper.includes('FROM DEMAND_RAW') && (upper.includes('DR.LEVEL = 3') || upper.includes('DR.LEVEL=3')) && (upper.includes('DR.LEVEL = 4') || upper.includes('DR.LEVEL=4'))) {
      const filtered = this.demand_raw.filter(d => (Number(d.level) === 3 || Number(d.level) === 4) && (d.status === 'PENDING' || d.status === 'APPROVED'));
      const rows = filtered.map(d => {
        const cust = this.customers.find(c => Number(c.customer_id) === Number(d.customer_id)) || {};
        const prod = this.products.find(p => Number(p.product_id) === Number(d.product_id)) || {};
        return {
    ...
    if (upper.includes('FROM DEMAND_RAW') && (upper.includes('DR.LEVEL =') || upper.includes('DR.LEVEL='))) {
      let lvl = 0;
      if (params.length > 0 && typeof params[0] === 'number') {
        lvl = Number(params[0]);
      } else {
        const match = upper.match(/DR\.LEVEL\s*=\s*(\d+)/);
        if (match) {
          lvl = parseInt(match[1], 10);
        }
      }
      const filtered = this.demand_raw.filter(d => Number(d.level) === lvl && (d.status === 'PENDING' || d.status === 'PARTIALLY_ALLOCATED' || d.status === 'APPROVED'));
      const rows = filtered.map(d => {
        const cust = this.customers.find(c => Number(c.customer_id) === Number(d.customer_id)) || {};
        const prod = this.products.find(p => Number(p.product_id) === Number(d.product_id)) || {};
        return {
    ...
    if (upper.includes('FROM DEMAND_RAW') && (upper.includes('CUSTOMER_ID = $') || upper.includes('WHERE CUSTOMER_ID =') || upper.includes('WHERE DR.CUSTOMER_ID =')) && !upper.includes('STATUS IN') && !upper.includes('PRODUCT_ID = $') && !upper.includes('PRODUCT_ID=$')) {
      ...
      const rows = filtered.map(d => {
        ...
        let officialAllocated = 0;
        if (isApproved) {
          const appr = this.demand_approved.find(a => Number(a.demand_id) === Number(d.demand_id));
          if (appr) {
            officialAllocated = Number(appr.fulfilled_quantity);
          } else {
            const allocations = this.supply_allocation.filter(a => Number(a.demand_id) === Number(d.demand_id));
            officialAllocated = allocations.reduce((sum, a) => sum + Number(a.allocated_quantity), 0) || Number(d.suggested_quantity) || Number(d.requested_quantity);
          }
        }
        return {
          ...d,
          ...
          product_name: prod.product_name,
          unit_price: prod.unit_price,
          allocated_quantity: officialAllocated,
          remaining_quantity: Number(d.requested_quantity) - officialAllocated
        };
    ...
    if (upper.startsWith('INSERT INTO DEMAND_RAW')) {
      ...
      let newDemand;
      if (params.length === 7) {
        // [customerId, productId, requestedQuantity, requiredDate, suggestedQuantity, reason, confidence]
        newDemand = {
          demand_id: id,
          ...
          level: 0,
          status: 'PENDING',
          suggested_quantity: params[4] !== undefined ? Number(params[4]) : null,
          reason: params[5] || null,
          confidence: params[6] !== undefined ? Number(params[6]) : null,
          created_at: new Date(),
          updated_at: new Date()
        };
      } else {
    ...
    if (upper.includes('FROM DEMAND_RAW') && (upper.includes('JOIN CUSTOMERS') || upper.includes('GROUP BY')) && !upper.includes('WHERE DR.LEVEL =') && !upper.includes('WHERE DR.CUSTOMER_ID =') && !upper.includes('WHERE CUSTOMER_ID =')) {
      const rows = this.demand_raw.map(d => {
        const prod = this.products.find(p => Number(p.product_id) === Number(d.product_id)) || {};
        const cust = this.customers.find(c => Number(c.customer_id) === Number(d.customer_id)) || {};
        const isApproved = d.status === 'APPROVED';
        let officialAllocated = 0;
    ...
    if (upper.startsWith('UPDATE DEMAND_RAW')) {
      ...
      if (item) {
        ...
        if (upper.includes("STATUS = 'APPROVED'")) item.status = 'APPROVED';
          ...
          if (typeof params[0] === 'string' && params[0] !== 'REJECTED') {
            item.reason = params[0];
          }
        }
    ...
    // ACTION LOG queries
    if (upper.includes('FROM DEMAND_ACTION_LOG') && (upper.includes('DEMAND_ID =') || upper.includes('DEMAND_ID='))) {
      const id = Number(params[0]);
      ...
      const rows = logs.map(l => {
        ...
        return {
          ...l,
          user_name: user.name || (user.role ? `${user.role} Approver` : 'System Reviewer'),
    ...
    if (upper.startsWith('INSERT INTO DEMAND_ACTION_LOG')) {
      ...
      if (upper.includes("'APPROVED'")) actionType = 'APPROVED';
      else if (upper.includes("'REJECTED'")) actionType = 'REJECTED';
      else if (upper.includes("'PARTIALLY_ALLOCATED'")) actionType = 'PARTIALLY_ALLOCATED';
      else if (upper.includes("'SUBMITTED'")) actionType = 'SUBMITTED';

      // Detect level from query string literals if present
      if (upper.includes(" 1, 'APPROVED'") || upper.includes(", 1, 'APPROVED'") || upper.includes(", 1, 'REJECTED'")) level = 1;
      else if (upper.includes(" 2, 'APPROVED'") || upper.includes(", 2, 'APPROVED'") || upper.includes(", 2, 'PARTIALLY_ALLOCATED'") || upper.includes(", 2, 'REJECTED'")) level = 2;
      else if (upper.includes(" 3, 'APPROVED'") || upper.includes(", 3, 'APPROVED'") || upper.includes(", 3, 'REJECTED'")) level = 3;
      else if (upper.includes(" 4, 'APPROVED'") || upper.includes(", 4, 'APPROVED'") || upper.includes(", 4, 'REJECTED'")) level = 4;

      const demandId = Number(params[0]);

      if (params.length === 7) {
        // [demand_id, level, action_type, approved_quantity, reason, comment, approved_by]
        level = Number(params[1]);
        actionType = params[2];
        approvedQty = params[3] !== undefined && params[3] !== null ? Number(params[3]) : null;
        reason = params[4] || null;
        comment = params[5] || null;
        approvedBy = params[6] ? Number(params[6]) : null;
      } else if (params.length === 6) {
        // [demand_id, level, action_type, reason, comment, approved_by]
        level = Number(params[1]);
        actionType = params[2];
        reason = params[3] || null;
        comment = params[4] || null;
        approvedBy = params[5] ? Number(params[5]) : null;
      } else if (params.length === 4) {
        if (actionType === 'REJECTED') {
          // [demand_id, reason, comment, user_id]
          reason = params[1] || null;
          comment = params[2] || null;
          approvedBy = params[3] ? Number(params[3]) : null;
        } else {
          // [demand_id, approved_quantity, comment, user_id]
          approvedQty = Number(params[1]) || null;
          comment = params[2] || null;
          approvedBy = params[3] ? Number(params[3]) : null;
        }
      } else if (params.length === 3) {
        // [demand_id, approved_quantity, user_id] or [demand_id, reason, user_id]
        if (typeof params[1] === 'number') {
          approvedQty = Number(params[1]);
        } else {
          reason = params[1] || null;
        }
        approvedBy = params[2] ? Number(params[2]) : null;
      }

      // If reason wasn't extracted from params, search in query string literal
      if (!reason) {
        const rMatch = text.match(/'([^']{10,})'/);
        if (rMatch) reason = rMatch[1];
      }
      ...
      const newLog = {
        ...
        reason: reason || (actionType === 'APPROVED' ? `Level ${level} Verification Passed` : actionType === 'REJECTED' ? `Level ${level} Rejected` : 'Submitted'),
    ...
    if (upper.includes('FROM DEMAND_APPROVED')) {
      if (params.length > 0) {
        const id = Number(params[0]);
        let rows = this.demand_approved;
        if (upper.includes('DEMAND_ID =') || upper.includes('DEMAND_ID=')) {
          rows = rows.filter(a => Number(a.demand_id) === id);
        } else {
          rows = rows.filter(a => Number(a.customer_id) === id);
        }
        return { rows: rows.map(r => ({ ...r })), rowCount: rows.length };
      }
