...
export const api = {
  ...
  listSupply: (productId, month) => {
    ...
    return request(`/admin/supply/list?${query.toString()}`);
  },
  addSupply: (supplyData) => request('/admin/supply/add', {
    method: 'POST',
    body: JSON.stringify(supplyData)
  }),
  listProducts: () => request('/admin/products/list'),
  ...
  // Customer
  getCustomerProducts: () => request('/customer/products'),
  ...
  approveLevel2: (demandId, comment, manualWeeks = null) => request(`/level2/demand/${demandId}/approve`, {
    method: 'POST',
    body: JSON.stringify({ comment, manualWeeks })
  }),
  ...
  approveLevel3: (demandId, comment) => request(`/level3/demand/${demandId}/approve`, {
    method: 'POST',
    body: JSON.stringify({ comment })
  }),
  ...
  getSupplyAvailability: (productId, month) => request(`/level2/supply/availability?product_id=${productId}&month=${month}`),
