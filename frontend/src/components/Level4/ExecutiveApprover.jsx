...
export default function ExecutiveApprover({ demand, onClose, onComplete }) {
  ...
  const handleApprove = async () => {
    if (hasExceptions && (!comment || comment.trim().length < 5)) {
      setError('An executive rationale comment (minimum 5 characters) is mandatory to override active exceptions.');
      return;
    }
    ...
    setError(null);
    try {
      await api.approveLevel4(demand.demand_id, comment, overrideExceptions);
      onComplete({
        success: true,
  ...
  const handleReject = async () => {
    ...
    setError(null);
    try {
      await api.rejectLevel4(demand.demand_id, rejectionReason, comment);
      onComplete({
        success: true,
        ...
        <div className="space-y-3">
          <div>
            <label className="block text-slate-400 font-semibold mb-1">
              Executive Resolution & Audit Authorization Note {hasExceptions ? '*' : '(Optional)'}
            </label>
            <textarea
              value={comment}
              onChange={(e) => setComment(e.target.value)}
              rows={3}
              placeholder={hasExceptions ? 'Mandatory: Explain business rationale for overriding exceptions...' : 'Optional fulfillment sign-off comment...'}
