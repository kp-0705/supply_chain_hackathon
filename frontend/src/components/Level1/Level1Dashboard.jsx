    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-[11px] font-mono">
        <div className="p-2 rounded-xl bg-slate-950/60 border border-slate-800 text-slate-400">
          <strong className="text-slate-200 block">Rule 1.1 & 1.2</strong>
          Customer Active & Credit Cap
        </div>
        <div className="p-2 rounded-xl bg-slate-950/60 border border-slate-800 text-slate-400">
          <strong className="text-slate-200 block">Rule 1.3 & 1.4</strong>
          Active Catalog & Positive Qty
        </div>
        <div className="p-2 rounded-xl bg-slate-950/60 border border-slate-800 text-slate-400">
          <strong className="text-slate-200 block">Rule 1.5 & 1.6</strong>
          &le;90-Day Date & Tier Match
        </div>
        <div className="p-2 rounded-xl bg-slate-950/60 border border-slate-800 text-slate-400">
          <strong className="text-slate-200 block">Rule 1.7</strong>
          No Duplicate Active Demands
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div className="glass-card rounded-2xl p-12 text-center space-y-2">
          ...
          <p className="text-xs text-slate-500">All customer demands have completed initial eligibility validation.</p>
        </div>
      ) : (
          ...
          {demands.map(d => (
            <div key={d.demand_id} className="space-y-2">
              <DemandCard
                ...
                actionButtonLabel="Approve & Send to L2"
      ...
      <Modal
        ...
        title={`Approve Demand #${approveDemand?.demand_id} (Level 1)`}
        footer={
          <>
            <button
            ...
            >
              {approving ? 'Advancing...' : 'Confirm Level 1 Approval'}
            </button>
          </>
        }
      >
        <div className="space-y-3 text-xs">
          ...
          <div>
            <label className="block text-slate-400 font-semibold mb-1">Approval Note / Audit Comment (Optional)</label>
            <textarea
              value={approveComment}
              onChange={(e) => setApproveComment(e.target.value)}
              rows={3}
              placeholder="e.g. Credit verification confirmed with Treasury team."
